/datum/erp_action/other/penis/hemi/dp_vag_anal
	...
	message_start = "{actor} {pose} приставляет свои члены сразу к обоим дырочкам {partner}."
	message_tick = "{actor} {pose}, {force} и {speed} трахает {partner} сразу в киску и зад."
	message_finish =  "{actor} вытаскивает члены из дырок {partner}."
	message_climax_active = "{actor} кончает одновременно в киску и зад {partner}."
	message_climax_passive = "{partner} кончает под себя!"
	climax_liquid_mode_active = "into"
	
/datum/erp_action/other/penis/hemi/dp_vag_anal/get_filter_target_organ_types()
	return list(required_target, required_target_second)

/datum/erp_action/other/penis/hemi/dp_vag_anal/proc/get_action_organs_dp(mob/living/carbon/human/user, mob/living/carbon/human/target, only_free_init = TRUE,	only_free_target = FALSE, only_free_second = FALSE)
...
/datum/erp_action/other/penis/hemi/dp_vag_anal/shows_on_menu(user, target)
	if(!..())
		return FALSE
...
/datum/erp_action/other/penis/hemi/dp_vag_anal/can_perform(user, target)
	. = ..()
	if(!user || !target)
		return FALSE
...
/datum/erp_action/other/penis/hemi/dp_vag_anal/connect_organs(user, target)
...
/datum/erp_action/other/penis/hemi/dp_vag_anal/get_reserved_target_organ_types()
	if(!reserve_target_for_session)
		return null

	return list(SEX_ORGAN_VAGINA, SEX_ORGAN_ANUS)

/datum/erp_action/other/penis/hemi/dp_vag_anal/get_knot_count()
	return 2
