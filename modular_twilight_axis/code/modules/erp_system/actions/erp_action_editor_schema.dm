...
/datum/erp_action_editor_schema/proc/export_editor_fields(datum/erp_action/A)
	. = list()
	. += list(_make_field("action_scope", "Направление действия", "enum", A.action_scope, "ОСНОВНОЕ", null, null, null, _scope_options(), "Self — действие с собой. Other — действие с партнёром. Это влияет на список доступных действий и фильтрацию.", null))
	. += list(_make_field("required_init_organ", "Орган актёра (init)", "enum", A.required_init_organ, "ОРГАНЫ", null, null, null, _organ_options()))
	...
	. += list(_make_field("tick_time", "Tick time (сек)", "number", _ticks_to_seconds(A.tick_time), "ТАЙМИНГ", 0.1, 60, 0.1))
	. += list(_make_field("continuous", "Непрерывное", "bool", A.continuous, "ТАЙМИНГ"))
	. += list(_make_field("active_arousal_coeff",  "Возбуждение актёра", "number", A.active_arousal_coeff,  "ЭФФЕКТЫ", 0, 10, 0.1))
	. += list(_make_field("passive_arousal_coeff", "Возбуждение цели",  "number", A.passive_arousal_coeff, "ЭФФЕКТЫ", 0, 10, 0.1))
	. += list(_make_field("active_pain_coeff",     "Боль актёра",       "number", A.active_pain_coeff,     "ЭФФЕКТЫ", 0, 10, 0.1))
	...
	. += list(_make_field("required_item_tags", "Нужные теги предмета", "string_list", A.required_item_tags, "ТЕГИ",
		null, null, null, null, "Напр: dildo. Если список не пуст — действие потребует предмет с одним из тегов.", "tag"))
	. += list(_make_field("action_tags", "Теги действия", "string_list", A.action_tags, "ТЕГИ",
	...
	. += list(_make_field("message_tick", "Сообщение: тик", "text", A.message_tick, "СООБЩЕНИЯ"))
	. += list(_make_field("message_finish", "Сообщение: финиш", "text", A.message_finish, "СООБЩЕНИЯ"))
	. += list(_make_field("message_climax_active", "Оргазм: актёр", "text", A.message_climax_active, "СООБЩЕНИЯ"))
